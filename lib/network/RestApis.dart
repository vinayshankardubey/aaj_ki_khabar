...
Future<List<TweetModel>> loadTweets() async {
  return await get(
    ...
    if (timelineRes.statusCode.isSuccessful()) {
      log("-------------------------------------->Step2 ${getStringAsync(TWITTER_USERNAME)}");
      retryCount = 0;
...
Future<LoginResponse> login(Map request, {bool isSocialLogin = false}) async {
  Response response = await buildHttpResponse(isSocialLogin ? '' : 'jwt-auth/v1/token', request: request, method: HttpMethod.POST);
...
Future<SearchNewsResponse> blogFilterNewsApi(Map? request, int page) async {
  return SearchNewsResponse.fromJson(await (handleResponse(await buildHttpResponse('', request: request, method: HttpMethod.POST))));
}
...
Future<DashboardResponse> getDashboardApi(Map request, int page) async {
  ...
  return await handleResponse(await buildHttpResponse('', request: request, method: HttpMethod.POST)).then((value) async {
...
Future<NewsData> getBlogDetail(Map request, bool isLogin) async {
  return NewsData.fromJson(await (handleResponse(await buildHttpResponse("", request: request, method: HttpMethod.POST))));
}
//endregion
...
Future<BR.BaseResponse> removeWishList(Map request) async {
  return BR.BaseResponse.fromJson(await (handleResponse(await buildHttpResponse('', request: request, method: HttpMethod.POST))));
}
...
Future<LikeResponse> blogLikeDisLike({Map? req}) async {
  return LikeResponse.fromJson(await (handleResponse(await buildHttpResponse('', request: req, method: HttpMethod.POST))));
}

Future<BR.BaseResponse> deleteAccount() async {
  return BR.BaseResponse.fromJson(await (handleResponse(await buildHttpResponse('', method: HttpMethod.POST))));
}
